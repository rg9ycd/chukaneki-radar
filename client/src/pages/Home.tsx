...
import { formatIndividualDistance } from "@/lib/resultDisplay";
...
export default function Home() {
  ...
  const addParticipant = () => { setResult(null); setParticipants(current => [...current, makeParticipant(current.length)]); };
  ...
  return <main className={`app-shell ${theme}`}>
    <header className="app-header">
      <div className="brand-block"><div className="brand-mark-frame" aria-label="中間駅レーダーのロゴ"><img className="brand-mark" src="/manus-storage/radar-logo_0dcf6967.png" alt="" onError={event => event.currentTarget.classList.add("asset-unavailable")} /><span className="brand-mark-fallback" aria-hidden="true"><i /><i /><i /><i /><b>◎</b></span></div><div><p className="eyebrow"><Radar size={13} /> COMMUTE RADAR</p><h1>中間駅<br /><em>レーダー</em></h1></div><button type="button" className="theme-toggle" onClick={() => setTheme(current => current === "dark" ? "light" : "dark")} aria-label={theme === "dark" ? "ライトモードへ切替" : "ダークモードへ切替"}>{theme === "dark" ? <Sun size={16} /> : <Moon size={16} />}<span>{theme === "dark" ? "LIGHT" : "DARK"}</span></button></div>
      <p className="brand-copy">みんなの移動線が重なる、集まりやすい場所を探す。</p>
    </header>
    ...
    <section className="operation-panel">
      ...
      <div className="calculate-actions"><button type="button" className="scan-button" disabled={isCalculating} onClick={calculate}>{isCalculating ? <LoaderCircle className="spin" size={18} /> : <ScanSearch size={18} />}{isCalculating ? "レーダーを走査中…" : meetingMode === "station" ? "重心に近い駅を探す" : meetingMode === "route" ? "通勤線バランスを算出" : "入力駅の重心を算出"}<ChevronRight size={17} /></button><button type="button" className="demo-button" disabled={isDemoLoading} onClick={loadDemo}>{isDemoLoading ? <LoaderCircle className="spin" size={15} /> : <MapPinned size={15} />} 体験データを入力</button></div>
    </section>
    <section className="members-panel"><div className="members-panel-header"><span><Users size={15} /> 参加メンバー</span><small>{participants.length}/10 MEMBERS</small></div><div className="member-list">{participants.map((participant, index) => <ParticipantCard key={participant.id} participant={participant} index={index} canRemove={participants.length > 2} onRemove={() => { setParticipants(current => current.filter(item => item.id !== participant.id)); setResult(null); }} onUpdate={update => updateParticipant(participant.id, update)} />)}</div><button type="button" className="add-member-button" disabled={participants.length >= 10} onClick={addParticipant}><Plus size={17} /> 参加者を追加 <span>{participants.length}/10</span></button></section>
    <footer className="rail-footer">駅データ：HeartRails Express　／　地図：OpenStreetMap contributors</footer><div className="mobile-safety"><CircleAlert size={15} /> 出発駅は必須、帰宅駅は任意です。駅候補をタップして確定してください。</div>
  </main>;
}
